...
// Real SSH connections to production servers via Tailscale mesh
#define TAILSCALE_IP "100.95.120.67"  // alexa-louise.taile5d081.ts.net
#define LUCIDIA_IP "192.168.4.38"      // Tailscale: 100.66.235.47
#define ALICE_IP "192.168.4.49"
#define ARIA_IP "192.168.4.64"
#define OCTAVIA_IP "192.168.4.74"
#define SHELLFISH_IP "174.138.44.45"  // DigitalOcean
...
#define COLOR_WARM         COLOR_AMBER
#define COLOR_DEEP_MAG     COLOR_MAGENTA
#define COLOR_VIVID_PUR    COLOR_VIOLET      // #9C27B0 (was #7700FF)

// Include dynamic navigation and sovereign stack AFTER color definitions
#include "dynamic_nav.h"       // Dynamic Navigation System
...
enum Screen {
  ...
  SCREEN_SOVEREIGN_STACK,  // Sovereign Stack Monitor - Infrastructure health
  SCREEN_INFRASTRUCTURE,   // Infrastructure Dashboard - Real-time system overview
  SCREEN_HOT_LEADS,        // Hot Leads - CRM top prospects with actionable intelligence
  SCREEN_CEO_COMMAND       // CEO Command Center - Ultimate aggregated dashboard
};
...
void drawAIInference() {
  ...
  // Title - AI OPERATOR DASHBOARD
  tft.setTextColor(COLOR_HOT_PINK);
  tft.setTextDatum(MC_DATUM);
  brFont.drawMonoText("AI OPERATOR", 120, 30, 3, COLOR_WHITE);

  // System metrics cards
  int cardY = 55;

  // Card 1: CPU/Memory
  drawCard(10, cardY, 70, 45, COLOR_VIVID_PUR);
  tft.setTextColor(COLOR_WHITE);
  tft.setTextDatum(TL_DATUM);
  brFont.drawMonoText("CPU", 18, cardY + 8, 1, COLOR_WHITE);
  tft.setTextColor(COLOR_VIVID_PUR);
  brFont.drawMonoText("240", 18, cardY + 23, 2, COLOR_WHITE);
  tft.setTextColor(COLOR_DARK_GRAY);
  brFont.drawMonoText("MHz", 18, cardY + 36, 1, COLOR_WHITE);

  // Card 2: RAM Usage
  drawCard(85, cardY, 70, 45, COLOR_HOT_PINK);
  tft.setTextColor(COLOR_WHITE);
  brFont.drawMonoText("RAM", 93, cardY + 8, 1, COLOR_WHITE);
  tft.setTextColor(COLOR_HOT_PINK);
  brFont.drawMonoText("320", 93, cardY + 23, 2, COLOR_WHITE);
  tft.setTextColor(COLOR_DARK_GRAY);
  brFont.drawMonoText("KB", 93, cardY + 36, 1, COLOR_WHITE);

  // Card 3: WiFi Signal
  drawCard(160, cardY, 70, 45, COLOR_CYBER_BLUE);
  tft.setTextColor(COLOR_WHITE);
  brFont.drawMonoText("WiFi", 168, cardY + 8, 1, COLOR_WHITE);
  tft.setTextColor(COLOR_CYBER_BLUE);
  int rssi = WiFi.status() == WL_CONNECTED ? WiFi.RSSI() : -100;
  brFont.drawMonoText(String(rssi).c_str(), 168, cardY + 23, 2, COLOR_WHITE);
  tft.setTextColor(COLOR_DARK_GRAY);
  brFont.drawMonoText("dBm", 168, cardY + 36, 1, COLOR_WHITE);

  // Operator Profile Card
  drawCard(10, 110, 220, 85, COLOR_DARK_GRAY);

  // Profile icon
  tft.fillCircle(35, 135, 16, COLOR_HOT_PINK);
  tft.setTextColor(COLOR_WHITE);
  tft.setTextDatum(MC_DATUM);
  brFont.drawMonoText("OP", 35, 135, 2, COLOR_WHITE);

  // Profile info
  tft.setTextColor(COLOR_HOT_PINK);
  tft.setTextDatum(TL_DATUM);
  brFont.drawMonoText("OPERATOR", 58, 120, 2, COLOR_WHITE);

  tft.setTextColor(COLOR_WHITE);
  brFont.drawMonoText("operator@blackroad.io", 58, 138, 1, COLOR_WHITE);
  brFont.drawMonoText("Edge AI Device", 58, 151, 1, COLOR_WHITE);

  // Status badges
  tft.fillRoundRect(18, 165, 70, 18, 4, COLOR_CYBER_BLUE);
  tft.setTextColor(COLOR_WHITE);
  tft.setTextDatum(MC_DATUM);
  brFont.drawMonoText("ONLINE", 53, 174, 1, COLOR_WHITE);

  tft.fillRoundRect(93, 165, 60, 18, 4, COLOR_VIVID_PUR);
  brFont.drawMonoText("READY", 123, 174, 1, COLOR_WHITE);

  tft.fillRoundRect(158, 165, 60, 18, 4, COLOR_SUNRISE);
  brFont.drawMonoText("SSH:22", 188, 174, 1, COLOR_WHITE);

  // Connection info
  tft.setTextColor(COLOR_CYBER_BLUE);
  tft.setTextDatum(TL_DATUM);
  brFont.drawMonoText("SSH TARGET", 15, 205, 2, COLOR_WHITE);

  tft.setTextColor(COLOR_WHITE);
  brFont.drawMonoText("octavia @ 192.168.4.38", 15, 223, 1, COLOR_WHITE);
  brFont.drawMonoText("Port: 22 | Auth: Key", 15, 238, 1, COLOR_WHITE);

  // Uptime
  unsigned long uptime = millis() / 1000;
  String uptimeStr = String(uptime / 60) + "m " + String(uptime % 60) + "s";
  tft.setTextColor(COLOR_DARK_GRAY);
  tft.setTextDatum(TR_DATUM);
  brFont.drawMonoText(("Up: " + uptimeStr).c_str(), 220, 238, 1, COLOR_WHITE);
...
void drawSettingsScreen() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    tft.drawString(ip.c_str(), 60, y+5, 1);
    int rssi = WiFi.RSSI();
    String rssiStr = String(rssi) + "dBm";
    tft.setTextColor(COLOR_SUNRISE);
    tft.drawString(rssiStr.c_str(), 160, y+5, 1);
    tft.fillCircle(302, y+9, 3, COLOR_CYBER_BLUE); // Connected
  } else {
    tft.drawString("Disconnected", 60, y+5, 1);
    tft.fillCircle(302, y+9, 3, COLOR_HOT_PINK); // Offline
  }
...
void drawCRMScreen() {
  ...
  tft.fillCircle(215, 31, 6, navState.crmHealthy ? COLOR_VIVID_PUR : COLOR_SUNRISE);
  tft.setTextColor(COLOR_BLACK);
  tft.drawString(navState.crmHealthy ? "L" : "S", 215, 28, 1);

  tft.setTextColor(COLOR_WHITE);
  tft.setTextDatum(TC_DATUM);
  tft.drawString("BlackRoad CRM", 160, 42, 1);

  // Pipeline header with LIVE DATA
  int y = 56;
  tft.setTextDatum(TL_DATUM);
  tft.setTextColor(COLOR_SUNRISE);
  ...
  // Use live CRM metrics
  String pipelineStr = "$" + String(crmMetrics.pipelineValue / 1000) + "K";
  pipelineStr += " (" + String(crmMetrics.totalContacts) + " contacts)";
  tft.drawString(pipelineStr, 60, y, 1);

  // Hot leads indicator
  tft.setTextColor(crmMetrics.hotLeads > 10 ? COLOR_HOT_PINK : COLOR_VIVID_PUR);
  tft.setTextDatum(TR_DATUM);
  tft.drawString(String(crmMetrics.hotLeads) + " hot", 306, y, 1);
...
void drawMeshVPN() {
  ...
  // Title with live status indicator
  tft.setTextColor(COLOR_CYBER_BLUE);
  tft.setTextDatum(TC_DATUM);
  tft.drawString("MESH VPN", 160, 27, 2);

  // LIVE/STATIC indicator
  tft.fillCircle(215, 31, 6, navState.meshHealthy ? COLOR_VIVID_PUR : COLOR_SUNRISE);
  tft.setTextColor(COLOR_BLACK);
  tft.drawString(navState.meshHealthy ? "L" : "S", 215, 28, 1);

  tft.setTextColor(COLOR_WHITE);
  tft.setTextDatum(TC_DATUM);
  tft.drawString("Tailscale Mesh", 160, 42, 1);

  // Network stats header with node count
  int y = 56;
  tft.setTextDatum(TL_DATUM);
  tft.setTextColor(COLOR_SUNRISE);
  tft.drawString("Network:", 10, y, 1);
  tft.setTextColor(COLOR_WHITE);
  tft.drawString("100.64.0.0/10", 70, y, 1);

  // Show active nodes
  tft.setTextColor(navState.activeNodes >= 3 ? COLOR_VIVID_PUR : COLOR_HOT_PINK);
  tft.setTextDatum(TR_DATUM);
  tft.drawString(String(navState.activeNodes) + " active", 306, y, 1);

  // Render dynamic mesh nodes
  y += 16;

  for (int i = 0; i < meshNodeCount && i < 4; i++) {
    tft.fillRoundRect(10, y, 300, 20, 4, COLOR_DARK_GRAY);

    // Node name - colored by status
    uint16_t nodeColor = COLOR_WHITE;
    if (meshNodes[i].status == "active") nodeColor = COLOR_VIVID_PUR;
    else if (meshNodes[i].status == "idle") nodeColor = COLOR_SUNRISE;
    else nodeColor = COLOR_HOT_PINK;

    tft.setTextColor(nodeColor);
    tft.setTextDatum(TL_DATUM);
    String nodeName = meshNodes[i].name;
    nodeName.toUpperCase();
    tft.drawString(nodeName.substring(0, 8), 14, y+4, 2);

    // IP address
    tft.setTextColor(COLOR_WHITE);
    String displayIP = meshNodes[i].ip;
    if (displayIP.length() > 13) displayIP = displayIP.substring(displayIP.length() - 13);
    tft.drawString(displayIP, 80, y+6, 1);

    // Latency
    tft.setTextColor(COLOR_SUNRISE);
    tft.drawString(String(meshNodes[i].latency) + "ms", 170, y+6, 1);

    // Bandwidth
    tft.setTextColor(COLOR_CYBER_BLUE);
    String bw = String(meshNodes[i].bandwidth, 1) + "MB/s";
    tft.drawString(bw, 215, y+6, 1);

    // Online status dot
    tft.fillCircle(302, y+10, 4, meshNodes[i].online ? nodeColor : COLOR_HOT_PINK);

    y += 24;
  }

  // Skip to next section if we have less than 4 nodes
  if (meshNodeCount < 4) {
    y += 24 * (4 - meshNodeCount);
  }

  // Network totals
  y += (meshNodeCount == 0 ? 0 : 28);
  if (meshNodeCount > 0) {
    ...
    for (int i = 0; i < meshNodeCount; i++) {
      totalBw += meshNodes[i].bandwidth;
    }
    String bwStr = String(totalBw, 1) + " MB/s";
    tft.drawString(bwStr, 220, y, 2);
  }

  // Bottom stats
  tft.setTextColor(COLOR_HOT_PINK); tft.setTextDatum(TC_DATUM);
  tft.drawString(String(navState.activeNodes) + " NODES", 100, 220, 1);
  tft.setTextColor(COLOR_WHITE);
  tft.drawString("|", 180, 220, 1);
  tft.setTextColor(navState.meshHealthy ? COLOR_VIVID_PUR : COLOR_SUNRISE);
  tft.drawString(navState.meshHealthy ? "LIVE" : "STATIC", 245, 220, 1);
...
void drawMessagesScreen() {
  ...
  // Unread badge
  tft.fillCircle(215, 31, 8, COLOR_HOT_PINK);
  tft.setTextColor(COLOR_BLACK);
  tft.drawString("3", 215, 28, 1);

  tft.setTextColor(COLOR_WHITE);
  tft.drawString("Matrix + Slack Bridge", 160, 42, 1);

  // Stats header with better formatting
  int y = 56;
  tft.setTextDatum(TL_DATUM);
  tft.setTextColor(COLOR_SUNRISE);
  tft.drawString("Today:", 10, y, 1);
  tft.setTextColor(COLOR_WHITE);
  tft.drawString("24 msgs", 50, y, 1);
  tft.setTextColor(COLOR_VIVID_PUR);
  tft.drawString("8 threads", 110, y, 1);
...
// PERFORMANCE MONITOR - Real-time system health and diagnostics
void drawMeta() {
  tft.fillScreen(COLOR_BLACK);
  drawStatusBar();
  ...
  tft.setTextColor(COLOR_CYBER_BLUE);
  tft.setTextDatum(TC_DATUM);
  brFont.drawMonoTextCentered("PERFORMANCE", 120, 27, BR_MONO_MEDIUM, COLOR_CYBER_BLUE);

  // Live indicator (pulsing dot)
  static bool pulse = false;
  pulse = !pulse;
  tft.fillCircle(215, 31, 6, pulse ? COLOR_VIVID_PUR : COLOR_HOT_PINK);
  brFont.drawMonoText("L", 215, 28, 1, COLOR_BLACK);
  ...
  // MEMORY HEALTH CARD
  // ═══════════════════════════════════════════════════════════════
  tft.fillRoundRect(8, y, 304, 45, 4, COLOR_DARK_GRAY);

  // Memory header
  brFont.drawTechnicalLabel("MEMORY", 14, y + 5, COLOR_HOT_PINK);

  // Heap usage
  String heapUsage = getHeapUsageString();
  brFont.drawMonoText(heapUsage.c_str(), 200, y + 5, 1, COLOR_WHITE);

  // Heap bar graph (full width)
  y += 18;
  int heapUsagePercent = getHeapUsagePercent();
  tft.fillRect(14, y, 280, 8, COLOR_BLACK);

  // Color based on usage (inverted - more free is better)
  uint16_t heapColor;
  if (heapUsagePercent < 50) heapColor = COLOR_CYBER_BLUE;      // < 50% used = good
  else if (heapUsagePercent < 70) heapColor = COLOR_SUNRISE;    // 50-70% used = warning
  else heapColor = COLOR_HOT_PINK;                              // > 70% used = critical

  int heapBarWidth = (heapUsagePercent * 280) / 100;
  tft.fillRect(14, y, heapBarWidth, 8, heapColor);

  // Memory stats (free, min free, fragmentation)
  y += 12;
  tft.setTextDatum(TL_DATUM);
  tft.setTextColor(COLOR_WHITE);
  char memStats[50];
  sprintf(memStats, "Free:%dK Min:%dK Frag:%d%%",
          perfMetrics.freeHeap / 1024,
          perfMetrics.minFreeHeap / 1024,
          perfMetrics.heapFragmentation);
  tft.drawString(memStats, 14, y, 1);
  ...
  // CPU header
  brFont.drawTechnicalLabel("CPU", 14, y + 5, COLOR_VIVID_PUR);

  // CPU frequency
  char cpuFreq[20];
  sprintf(cpuFreq, "%.0fMHz", getCPUFrequencyMHz());
  brFont.drawMonoText(cpuFreq, 200, y + 5, 1, COLOR_WHITE);
  ...
  tft.setTextColor(COLOR_WHITE);
  char loopStats[50];
  sprintf(loopStats, "Loop:%luus Avg:%luus Max:%luus",
          perfMetrics.loopTime / 1000,
          perfMetrics.avgLoopTime / 1000,
          perfMetrics.maxLoopTime / 1000);
  tft.drawString(loopStats, 14, y, 1);

  // Loops per second
  y += 12;
  char loopRate[30];
  sprintf(loopRate, "%.0f loops/sec", perfMetrics.loopsPerSecond);
  tft.drawString(loopRate, 14, y, 1);
  ...
  // WIFI QUALITY CARD
  // ═══════════════════════════════════════════════════════════════
  tft.fillRoundRect(8, y, 304, 34, 4, COLOR_DARK_GRAY);

  // WiFi header
  brFont.drawTechnicalLabel("WIFI", 14, y + 5, COLOR_SUNRISE);

  // WiFi quality
  String wifiQuality = getWiFiQualityString();
  uint16_t wifiColor = getWiFiQualityColor();
  brFont.drawMonoText(wifiQuality.c_str(), 200, y + 5, 1, wifiColor);
  ...
  tft.setTextColor(COLOR_WHITE);
  char wifiStats[40];
  sprintf(wifiStats, "RSSI:%ddBm Quality:%d%% SSID:%s",
          perfMetrics.rssi,
          perfMetrics.wifiQuality,
          WIFI_SSID);
  String wifiStr = String(wifiStats);
  if (wifiStr.length() > 38) {
    wifiStr = wifiStr.substring(0, 38) + "...";
  }
  tft.drawString(wifiStr.c_str(), 14, y, 1);

  y += 20;

  // ═══════════════════════════════════════════════════════════════
  // SYSTEM UPTIME & BOOT REASON
  // ═══════════════════════════════════════════════════════════════
  tft.fillRoundRect(8, y, 304, 26, 4, COLOR_DARK_GRAY);

  // Uptime
  tft.setTextDatum(TL_DATUM);
  tft.setTextColor(COLOR_WARM);
  tft.drawString("UPTIME:", 14, y + 5, 1);
  tft.setTextColor(COLOR_WHITE);
  String uptime = getUptimeString();
  tft.drawString(uptime.c_str(), 60, y + 5, 1);

  // Boot reason
  tft.setTextColor(COLOR_WARM);
  tft.drawString("BOOT:", 14, y + 15, 1);
  tft.setTextColor(COLOR_WHITE);
  String bootReason = getRebootReasonString();
  tft.drawString(bootReason.c_str(), 50, y + 15, 1);
  ...
  if (isLowMemory()) {
    tft.fillRoundRect(8, y, 304, 16, 4, COLOR_HOT_PINK);
    tft.setTextColor(COLOR_BLACK);
    tft.drawString("⚠ LOW MEMORY WARNING", 160, y + 4, 1);
    y += 20;
  }

  if (isHighFragmentation()) {
    tft.fillRoundRect(8, y, 304, 16, 4, COLOR_SUNRISE);
    tft.setTextColor(COLOR_BLACK);
    tft.drawString("⚠ HIGH FRAGMENTATION WARNING", 160, y + 4, 1);
    y += 20;
  }
  ...
  if (isSlowLoop()) {
    tft.fillRoundRect(8, y, 304, 16, 4, COLOR_SUNRISE);
    tft.setTextColor(COLOR_BLACK);
    tft.drawString("⚠ SLOW LOOP WARNING", 160, y + 4, 1);
    y += 20;
  }
  ...
  if (isWiFiWeak()) {
    tft.fillRoundRect(8, y, 304, 16, 4, COLOR_SUNRISE);
    tft.setTextColor(COLOR_BLACK);
    tft.drawString("⚠ WEAK WIFI SIGNAL", 160, y + 4, 1);
    y += 20;
  }

  // All systems nominal
  if (!isLowMemory() && !isHighFragmentation() && !isSlowLoop() && !isWiFiWeak()) {
    tft.fillRoundRect(8, y, 304, 16, 4, COLOR_CYBER_BLUE);
    tft.setTextColor(COLOR_BLACK);
    tft.drawString("✓ ALL SYSTEMS NOMINAL", 160, y + 4, 1);
  }
...
void drawWeather() {
  ...
  tft.setTextColor(COLOR_WHITE);
  tft.setTextDatum(TL_DATUM);
  brFont.drawMonoText("San Francisco, CA", 20, 58, 2, COLOR_WHITE);

  tft.setTextColor(COLOR_CYBER_BLUE);
  brFont.drawMonoText("72°F", 20, 78, 4, COLOR_WHITE);

  tft.setTextColor(COLOR_WHITE);
  brFont.drawMonoText("Partly Cloudy", 20, 108, 2, COLOR_WHITE);

  // Weather details
  tft.setTextColor(COLOR_DARK_GRAY);
  brFont.drawMonoText("Humidity: 65%", 180, 68, 1, COLOR_WHITE);
  brFont.drawMonoText("Wind: 12 mph NW", 180, 83, 1, COLOR_WHITE);
  brFont.drawMonoText("UV Index: 6", 180, 98, 1, COLOR_WHITE);
  brFont.drawMonoText("Feels: 70°F", 180, 113, 1, COLOR_WHITE);

  // 5-day forecast
  tft.setTextColor(COLOR_CYBER_BLUE);
  tft.setTextDatum(TL_DATUM);
  brFont.drawMonoText("5-DAY FORECAST", 15, 140, 2, COLOR_WHITE);
  ...
  tft.setTextColor(COLOR_DARK_GRAY);
  tft.setTextDatum(TC_DATUM);
  brFont.drawMonoText("Updated: 2 min ago", 160, 302, 1, COLOR_WHITE);
...
void drawGitHub() {
  ...
  tft.setTextColor(COLOR_VIVID_PUR);
  tft.setTextDatum(TC_DATUM);
  brFont.drawMonoText("GITHUB", 120, 28, 3, COLOR_WHITE);
  ...
  // Repos
  drawCard(10, cardY, 52, 42, COLOR_VIVID_PUR);
  tft.setTextColor(COLOR_WHITE);
  tft.setTextDatum(TL_DATUM);
...
void drawLinear() {
  ...
  tft.setTextColor(COLOR_HOT_PINK);
  tft.setTextDatum(TC_DATUM);
  brFont.drawMonoText("LINEAR", 160, 28, 3, COLOR_WHITE);

  // Team info
  tft.setTextColor(COLOR_WHITE);
  tft.setTextDatum(TL_DATUM);
  brFont.drawMonoText("BlackRoad Engineering", 15, 55, 2, COLOR_WHITE);

  tft.setTextColor(COLOR_DARK_GRAY);
  brFont.drawMonoText("Sprint 23 • 12 tasks", 15, 73, 1, COLOR_WHITE);
  ...
  tft.setTextColor(COLOR_HOT_PINK);
  tft.setTextDatum(TL_DATUM);
  brFont.drawMonoText("IN PROGRESS", 15, 150, 2, COLOR_WHITE);
  ...
  tft.setTextColor(COLOR_SUNRISE);
  tft.setTextDatum(TC_DATUM);
  brFont.drawMonoText("Velocity: 18 pts/week", 160, 302, 1, COLOR_WHITE);

  drawBottomNav();
}

// ═══════════════════════════════════════════════════════════════════
// SOVEREIGN STACK MONITOR - Infrastructure Health Dashboard
// ═══════════════════════════════════════════════════════════════════

void drawSovereignStack() {
  tft.fillScreen(COLOR_BLACK);
  drawStatusBar();
  drawBackButton();
  ...
  String status = getSovereigntyStatus();
  uint16_t statusColor = getSovereigntyColor();

  // Title
  tft.setTextColor(statusColor);
  tft.setTextDatum(TC_DATUM);
  brFont.drawMonoText("SOVEREIGNTY", 160, 28, BR_MONO_MEDIUM, statusColor);

  // Status badge
  tft.fillRoundRect(60, 48, 200, 20, 10, statusColor);
  tft.setTextColor(COLOR_BLACK);
  tft.setTextDatum(TC_DATUM);
  brFont.drawMonoText(status.c_str(), 160, 52, BR_MONO_SMALL, COLOR_BLACK);

  // Metrics cards
  int cardY = 75;

  // Sovereignty Score
  drawCard(10, cardY, 70, 42, statusColor);
  tft.setTextColor(COLOR_WHITE);
  tft.setTextDatum(TC_DATUM);
  brFont.drawMonoText("SCORE", 45, cardY + 6, 1, COLOR_WHITE);
  String scoreStr = String((int)metrics.sovereigntyScore) + "%";
  brFont.drawMonoText(scoreStr.c_str(), 45, cardY + 20, 2, COLOR_WHITE);

  // Total Components
  drawCard(85, cardY, 70, 42, COLOR_CYBER_BLUE);
  brFont.drawMonoText("TOTAL", 120, cardY + 6, 1, COLOR_WHITE);
  brFont.drawMonoText(String(metrics.totalComponents).c_str(), 120, cardY + 20, 2, COLOR_WHITE);

  // Sovereign Count
  drawCard(160, cardY, 70, 42, COLOR_VIVID_PUR);
  brFont.drawMonoText("SOVER", 195, cardY + 6, 1, COLOR_WHITE);
  brFont.drawMonoText(String(metrics.sovereignComponents).c_str(), 195, cardY + 20, 2, COLOR_WHITE);

  // Active Count
  drawCard(235, cardY, 75, 42, COLOR_SUNRISE);
  brFont.drawMonoText("ACTIVE", 272, cardY + 6, 1, COLOR_WHITE);
  brFont.drawMonoText(String(metrics.activeComponents).c_str(), 272, cardY + 20, 2, COLOR_WHITE);

  // Component list
  tft.setTextColor(COLOR_HOT_PINK);
  tft.setTextDatum(TL_DATUM);
  brFont.drawMonoText("STACK COMPONENTS", 15, 130, 1, COLOR_WHITE);

  int y = 148;
  int shown = 0;

  for (int i = 0; i < stackComponentCount && shown < 6; i++) {
    StackComponent comp = sovereignStack[i];

    // Skip non-sovereign for space
    if (!comp.sovereign) continue;

    // Status dot
    uint16_t dotColor = COLOR_DARK_GRAY;
    if (comp.status == "active") dotColor = COLOR_VIVID_PUR;
    else if (comp.status == "degraded") dotColor = COLOR_SUNRISE;
    else if (comp.status == "offline") dotColor = COLOR_HOT_PINK;

    tft.fillCircle(18, y + 4, 3, dotColor);

    // Component name
    tft.setTextColor(COLOR_WHITE);
    String name = comp.name;
    if (name.length() > 12) name = name.substring(0, 12);
    brFont.drawMonoText(name.c_str(), 30, y, 1, COLOR_WHITE);

    // Category
    tft.setTextColor(COLOR_CYBER_BLUE);
    String cat = comp.category;
    if (cat.length() > 8) cat = cat.substring(0, 8);
    brFont.drawMonoText(cat.c_str(), 130, y, 1, COLOR_WHITE);

    // License
    tft.setTextColor(isForkableLicense(comp.license) ? COLOR_VIVID_PUR : COLOR_SUNRISE);
    String lic = comp.license;
    if (lic.length() > 10) lic = lic.substring(0, 10);
    brFont.drawMonoText(lic.c_str(), 200, y, 1, COLOR_WHITE);
  ...
  tft.setTextColor(COLOR_SUNRISE);
  tft.setTextDatum(TC_DATUM);
  int forkable = getForkableCount();
  String footerStr = String(forkable) + "/" + String(metrics.sovereignComponents) + " forkable • ";
  footerStr += String(metrics.degradedComponents) + " degraded";
  brFont.drawMonoText(footerStr.c_str(), 160, 302, 1, COLOR_WHITE);

  drawBottomNav();
}

// ═══════════════════════════════════════════════════════════════════
// INFRASTRUCTURE DASHBOARD - Real-time System Overview
// ═══════════════════════════════════════════════════════════════════

void drawInfrastructureDashboard() {
  tft.fillScreen(COLOR_BLACK);
  drawStatusBar();
  drawBackButton();
  ...
  tft.setTextColor(COLOR_SUNRISE);
  tft.setTextDatum(TC_DATUM);
  brFont.drawMonoText("INFRASTRUCTURE", 160, 28, BR_MONO_MEDIUM, COLOR_SUNRISE);

  // Last update timestamp
  unsigned long timeSinceUpdate = (millis() - navState.lastUpdate) / 1000;
  String updateStr = String(timeSinceUpdate) + "s ago";
  tft.setTextColor(COLOR_DARK_GRAY);
  tft.setTextDatum(TC_DATUM);
  tft.drawString(updateStr, 160, 46, 1);

  // ─── SYSTEM STATUS GRID ───
  int y = 65;

  // Mesh Network Status
  drawCard(10, y, 145, 32, navState.meshHealthy ? COLOR_VIVID_PUR : COLOR_SUNRISE);
  tft.fillCircle(22, y + 16, 6, navState.meshHealthy ? COLOR_VIVID_PUR : COLOR_SUNRISE);
  brFont.drawMonoText(navState.meshHealthy ? "L" : "S", 22, y + 13, 1, COLOR_BLACK);
  tft.setTextColor(COLOR_WHITE);
  tft.setTextDatum(TL_DATUM);
  brFont.drawMonoText("MESH", 35, y + 6, 1, COLOR_WHITE);
  String nodesStr = String(navState.activeNodes) + " nodes";
  brFont.drawMonoText(nodesStr.c_str(), 35, y + 18, 1, COLOR_WHITE);

  // CRM Status
  drawCard(165, y, 145, 32, navState.crmHealthy ? COLOR_VIVID_PUR : COLOR_SUNRISE);
  tft.fillCircle(177, y + 16, 6, navState.crmHealthy ? COLOR_VIVID_PUR : COLOR_SUNRISE);
  brFont.drawMonoText(navState.crmHealthy ? "L" : "S", 177, y + 13, 1, COLOR_BLACK);
  brFont.drawMonoText("CRM", 190, y + 6, 1, COLOR_WHITE);
  uint16_t leadColor = navState.hotLeads > 10 ? COLOR_HOT_PINK : COLOR_WHITE;
  tft.setTextColor(leadColor);
  String hotStr = String(navState.hotLeads) + " hot";
  brFont.drawMonoText(hotStr.c_str(), 190, y + 18, 1, leadColor);

  y += 40;

  // AI Status
  drawCard(10, y, 145, 32, navState.aiHealthy ? COLOR_VIVID_PUR : COLOR_SUNRISE);
  tft.fillCircle(22, y + 16, 6, navState.aiHealthy ? COLOR_VIVID_PUR : COLOR_SUNRISE);
  brFont.drawMonoText(navState.aiHealthy ? "L" : "S", 22, y + 13, 1, COLOR_BLACK);
  tft.setTextColor(COLOR_WHITE);
  brFont.drawMonoText("AI", 35, y + 6, 1, COLOR_WHITE);
  String reqsStr = String(navState.aiRequests) + " reqs";
  brFont.drawMonoText(reqsStr.c_str(), 35, y + 18, 1, COLOR_WHITE);

  // Sovereignty Status
  SovereigntyMetrics sovMetrics = getSovereigntyMetrics();
  String sovStatus = getSovereigntyStatus();
  uint16_t sovColor = getSovereigntyColor();
  drawCard(165, y, 145, 32, sovColor);
  brFont.drawMonoText("SOVEREIGN", 177, y + 6, 1, COLOR_WHITE);
  String scoreStr = String((int)sovMetrics.sovereigntyScore) + "%";
  brFont.drawMonoText(scoreStr.c_str(), 177, y + 18, 1, COLOR_WHITE);
  ...
  tft.setTextColor(COLOR_VIVID_PUR);
  tft.setTextDatum(TL_DATUM);
  brFont.drawMonoText("MESH NODES", 15, y, 1, COLOR_WHITE);
  y += 16;

  for (int i = 0; i < meshNodeCount && i < 3; i++) {
    uint16_t nodeColor = COLOR_WHITE;
    if (meshNodes[i].status == "active") nodeColor = COLOR_VIVID_PUR;
    else if (meshNodes[i].status == "idle") nodeColor = COLOR_SUNRISE;

    tft.fillCircle(18, y + 6, 4, meshNodes[i].online ? nodeColor : COLOR_HOT_PINK);

    String nodeName = meshNodes[i].name;
    nodeName.toUpperCase();
    if (nodeName.length() > 10) nodeName = nodeName.substring(0, 10);

    tft.setTextColor(COLOR_WHITE);
    brFont.drawMonoText(nodeName.c_str(), 30, y + 2, 1, COLOR_WHITE);

    tft.setTextColor(COLOR_CYBER_BLUE);
    String ipStr = meshNodes[i].ip;
    if (ipStr.length() > 14) ipStr = ipStr.substring(ipStr.length() - 14);
    brFont.drawMonoText(ipStr.c_str(), 110, y + 2, 1, COLOR_CYBER_BLUE);

    tft.setTextColor(COLOR_SUNRISE);
    String latencyStr = String(meshNodes[i].latency) + "ms";
    brFont.drawMonoText(latencyStr.c_str(), 240, y + 2, 1, COLOR_SUNRISE);
  ...
  // ─── CRM PIPELINE ───
  tft.setTextColor(COLOR_HOT_PINK);
  brFont.drawMonoText("CRM PIPELINE", 15, y, 1, COLOR_WHITE);
  y += 16;

  String pipelineStr = "$" + String(crmMetrics.pipelineValue / 1000) + "K";
  tft.setTextColor(COLOR_VIVID_PUR);
  brFont.drawMonoText(pipelineStr.c_str(), 18, y, 2, COLOR_VIVID_PUR);

  tft.setTextColor(COLOR_WHITE);
  String contactsStr = String(crmMetrics.totalContacts) + " contacts";
  brFont.drawMonoText(contactsStr.c_str(), 100, y + 2, 1, COLOR_WHITE);
  String dealsStr = String(crmMetrics.openDeals) + " deals";
  brFont.drawMonoText(dealsStr.c_str(), 100, y + 14, 1, COLOR_WHITE);

  tft.setTextColor(navState.hotLeads > 10 ? COLOR_HOT_PINK : COLOR_SUNRISE);
  String hotLeadsStr = String(crmMetrics.hotLeads) + " HOT";
  brFont.drawMonoText(hotLeadsStr.c_str(), 220, y + 8, 1, navState.hotLeads > 10 ? COLOR_HOT_PINK : COLOR_SUNRISE);
  ...
  tft.setTextDatum(TC_DATUM);
  String healthStr = "";
  int healthyCount = (navState.meshHealthy ? 1 : 0) + (navState.crmHealthy ? 1 : 0) + (navState.aiHealthy ? 1 : 0);
  healthStr = String(healthyCount) + "/3 systems live • ";
  healthStr += String(sovMetrics.activeComponents) + "/" + String(sovMetrics.totalComponents) + " active";
  brFont.drawMonoText(healthStr.c_str(), 160, 302, 1, COLOR_WHITE);

  drawBottomNav();
}

// HOT LEADS - Top 5 prospects with actionable intelligence
void drawHotLeads() {
  tft.fillScreen(COLOR_BLACK);
  drawStatusBar();
  ...
  tft.setTextColor(COLOR_HOT_PINK);
  tft.setTextDatum(TC_DATUM);
  brFont.drawMonoTextCentered("HOT LEADS", 120, 27, BR_MONO_MEDIUM, COLOR_HOT_PINK);

  // Live/Static indicator
  tft.fillCircle(215, 31, 6, navState.crmHealthy ? COLOR_VIVID_PUR : COLOR_SUNRISE);
  brFont.drawMonoText(navState.crmHealthy ? "L" : "S", 215, 28, 1, COLOR_BLACK);

  // Subtitle
  tft.setTextColor(COLOR_SUNRISE);
  char subtitle[40];
  sprintf(subtitle, "Top %d • Ready to Close", hotLeadCount);
  brFont.drawTechnicalLabel(subtitle, 50, 42, COLOR_CYBER_BLUE);

  int y = 58;

  // Draw top 5 hot leads
  for (int i = 0; i < hotLeadCount && i < 5; i++) {
    HotLead& lead = hotLeads[i];

    // Lead card
    uint16_t cardColor = COLOR_DARK_GRAY;
    tft.fillRoundRect(8, y, 304, 46, 4, cardColor);

    // Score badge (top right)
    uint16_t scoreColor;
    if (lead.score >= 90) scoreColor = COLOR_HOT_PINK;       // Burning
    else if (lead.score >= 80) scoreColor = COLOR_VIVID_PUR; // Hot
    else scoreColor = COLOR_CYBER_BLUE;                       // Warm

    tft.fillCircle(290, y + 10, 12, scoreColor);
    tft.setTextColor(COLOR_BLACK);
    tft.setTextDatum(TC_DATUM);
    char scoreStr[4];
    sprintf(scoreStr, "%d", lead.score);
    tft.drawString(scoreStr, 290, y + 5, 2);

    // Name (bold, large)
    tft.setTextColor(COLOR_WHITE);
    tft.setTextDatum(TL_DATUM);
    brFont.drawMonoText(lead.name, 14, y + 5, 1, COLOR_WHITE);

    // Company (below name)
    tft.setTextColor(COLOR_CYBER_BLUE);
    brFont.drawMonoText(lead.company, 14, y + 16, 1, COLOR_CYBER_BLUE);

    // Stage badge
    tft.setTextColor(COLOR_SUNRISE);
    brFont.drawMonoText(lead.stage, 14, y + 27, 1, COLOR_SUNRISE);

    // Activity metrics (opens/clicks)
    tft.setTextColor(COLOR_WHITE);
    char metricsStr[40];
    sprintf(metricsStr, "%d opens • %d clicks", lead.opens, lead.clicks);
    brFont.drawMonoText(metricsStr, 14, y + 37, 1, COLOR_WARM);

    // Last activity (top right, below score)
    tft.setTextColor(COLOR_WARM);
    tft.setTextDatum(TR_DATUM);
    brFont.drawMonoText(lead.lastActivity, 270, y + 24, 1, COLOR_WARM);

    // Reply indicator
    if (lead.hasReplied) {
      tft.fillCircle(270, y + 38, 3, COLOR_VIVID_PUR);
      tft.setTextColor(COLOR_VIVID_PUR);
      brFont.drawMonoText("Replied", 240, y + 35, 1, COLOR_VIVID_PUR);
    }
  ...
  tft.setTextDatum(TC_DATUM);
  char summaryStr[60];
  sprintf(summaryStr, "%d total hot leads • $%dK pipeline",
          crmMetrics.hotLeads, crmMetrics.pipelineValue / 1000);
  brFont.drawMonoText(summaryStr, 160, y + 2, 1, COLOR_WHITE);

  // Refresh button (bottom right)
  tft.fillRoundRect(250, 278, 62, 32, 4, COLOR_VIVID_PUR);
  tft.setTextColor(COLOR_WHITE);
  tft.setTextDatum(TC_DATUM);
  brFont.drawMonoText("SYNC", 281, 288, 1, COLOR_WHITE);
  brFont.drawMonoText("CRM", 281, 297, 1, COLOR_WHITE);

  drawBottomNav();
}

// CEO COMMAND CENTER - Ultimate aggregated dashboard
void drawCEOCommandCenter() {
  tft.fillScreen(COLOR_BLACK);
  drawStatusBar();
  ...
  tft.setTextColor(COLOR_HOT_PINK);
  tft.setTextDatum(TC_DATUM);
  brFont.drawMonoTextCentered("COMMAND CENTER", 120, 27, BR_MONO_MEDIUM, COLOR_HOT_PINK);

  // Live indicators for all systems
  int indicatorY = 31;
  tft.fillCircle(195, indicatorY, 3, navState.meshHealthy ? COLOR_VIVID_PUR : COLOR_SUNRISE);
  tft.fillCircle(205, indicatorY, 3, navState.crmHealthy ? COLOR_VIVID_PUR : COLOR_SUNRISE);
  tft.fillCircle(215, indicatorY, 3, navState.aiHealthy ? COLOR_VIVID_PUR : COLOR_SUNRISE);

  int y = 48;

  // ═══════════════════════════════════════════════════════════
  // TOP PRIORITY METRICS (BIG NUMBERS)
  // ═══════════════════════════════════════════════════════════

  // CRM Pipeline Value (HUGE)
  tft.setTextColor(COLOR_HOT_PINK);
  tft.setTextDatum(TC_DATUM);
  char pipelineStr[20];
  sprintf(pipelineStr, "$%dK", crmMetrics.pipelineValue / 1000);
  tft.drawString(pipelineStr, 80, y, 4);  // Large font

  // Hot Leads Count (HUGE)
  tft.setTextColor(COLOR_SUNRISE);
  char hotLeadsStr[10];
  sprintf(hotLeadsStr, "%d", crmMetrics.hotLeads);
  tft.drawString(hotLeadsStr, 200, y, 4);  // Large font

  // Labels
  y += 28;
  tft.setTextColor(COLOR_CYBER_BLUE);
  tft.setTextDatum(TC_DATUM);
  brFont.drawMonoText("PIPELINE", 80, y, 1, COLOR_CYBER_BLUE);
  brFont.drawMonoText("HOT", 200, y, 1, COLOR_SUNRISE);

  y += 16;

  // ═══════════════════════════════════════════════════════════
  // INFRASTRUCTURE STATUS (3-COLUMN GRID)
  // ═══════════════════════════════════════════════════════════

  // Mesh Network
  tft.fillRoundRect(8, y, 96, 32, 4, COLOR_DARK_GRAY);
  tft.setTextDatum(TL_DATUM);
  tft.setTextColor(COLOR_CYBER_BLUE);
  brFont.drawMonoText("MESH", 12, y + 4, 1, COLOR_CYBER_BLUE);
  tft.setTextColor(COLOR_WHITE);
  char meshStr[10];
  sprintf(meshStr, "%d nodes", navState.activeNodes);
  brFont.drawMonoText(meshStr, 12, y + 15, 1, COLOR_WHITE);
  tft.fillCircle(90, y + 16, 4, navState.meshHealthy ? COLOR_VIVID_PUR : COLOR_HOT_PINK);

  // CRM System
  tft.fillRoundRect(112, y, 96, 32, 4, COLOR_DARK_GRAY);
  tft.setTextColor(COLOR_MAGENTA);
  brFont.drawMonoText("CRM", 116, y + 4, 1, COLOR_MAGENTA);
  tft.setTextColor(COLOR_WHITE);
  char crmStr[15];
  sprintf(crmStr, "%d deals", crmMetrics.openDeals);
  brFont.drawMonoText(crmStr, 116, y + 15, 1, COLOR_WHITE);
  tft.fillCircle(194, y + 16, 4, navState.crmHealthy ? COLOR_VIVID_PUR : COLOR_HOT_PINK);

  // AI Infrastructure
  tft.fillRoundRect(216, y, 96, 32, 4, COLOR_DARK_GRAY);
  tft.setTextColor(COLOR_VIVID_PUR);
  brFont.drawMonoText("AI", 220, y + 4, 1, COLOR_VIVID_PUR);
  tft.setTextColor(COLOR_WHITE);
  char aiStr[15];
  sprintf(aiStr, "%d req", navState.aiRequests);
  brFont.drawMonoText(aiStr, 220, y + 15, 1, COLOR_WHITE);
  tft.fillCircle(298, y + 16, 4, navState.aiHealthy ? COLOR_VIVID_PUR : COLOR_HOT_PINK);

  y += 38;

  // ═══════════════════════════════════════════════════════════
  // TOP HOT LEAD (ACTIONABLE)
  // ═══════════════════════════════════════════════════════════

  if (hotLeadCount > 0) {
    HotLead& topLead = hotLeads[0];

    tft.fillRoundRect(8, y, 304, 38, 4, COLOR_DARK_GRAY);

    // Score badge
    uint16_t scoreColor = (topLead.score >= 90) ? COLOR_HOT_PINK : COLOR_VIVID_PUR;
    tft.fillCircle(290, y + 10, 10, scoreColor);
    tft.setTextColor(COLOR_BLACK);
    tft.setTextDatum(TC_DATUM);
    char topScoreStr[4];
    sprintf(topScoreStr, "%d", topLead.score);
    tft.drawString(topScoreStr, 290, y + 5, 1);

    // Name + Company
    tft.setTextDatum(TL_DATUM);
    tft.setTextColor(COLOR_WHITE);
    brFont.drawMonoText(topLead.name, 12, y + 5, 1, COLOR_WHITE);
    tft.setTextColor(COLOR_CYBER_BLUE);
    brFont.drawMonoText(topLead.company, 12, y + 16, 1, COLOR_CYBER_BLUE);

    // Activity + Stage
    tft.setTextColor(COLOR_WARM);
    char topMetricsStr[30];
    sprintf(topMetricsStr, "%d/%d • %s", topLead.opens, topLead.clicks, topLead.stage);
    brFont.drawMonoText(topMetricsStr, 12, y + 27, 1, COLOR_WARM);

    y += 42;
  }

  // ═══════════════════════════════════════════════════════════
  // PERFORMANCE METRICS (2-COLUMN)
  // ═══════════════════════════════════════════════════════════

  // Memory Health
  tft.fillRoundRect(8, y, 148, 26, 4, COLOR_DARK_GRAY);
  tft.setTextDatum(TL_DATUM);
  tft.setTextColor(COLOR_HOT_PINK);
  brFont.drawMonoText("MEM", 12, y + 4, 1, COLOR_HOT_PINK);
  tft.setTextColor(COLOR_WHITE);
  char memStr[20];
  sprintf(memStr, "%d%% • %dK", getHeapUsagePercent(), perfMetrics.freeHeap / 1024);
  brFont.drawMonoText(memStr, 12, y + 14, 1, COLOR_WHITE);

  // WiFi Quality
  tft.fillRoundRect(164, y, 148, 26, 4, COLOR_DARK_GRAY);
  tft.setTextColor(COLOR_SUNRISE);
  brFont.drawMonoText("WIFI", 168, y + 4, 1, COLOR_SUNRISE);
  tft.setTextColor(COLOR_WHITE);
  char wifiStr[20];
  sprintf(wifiStr, "%d%% • %ddBm", perfMetrics.wifiQuality, perfMetrics.rssi);
  brFont.drawMonoText(wifiStr, 168, y + 14, 1, COLOR_WHITE);
  ...
  tft.fillRoundRect(8, y, 304, 28, 4, COLOR_DARK_GRAY);
  tft.setTextDatum(TL_DATUM);
  tft.setTextColor(COLOR_VIVID_PUR);
  brFont.drawMonoText("SOVEREIGNTY", 12, y + 4, 1, COLOR_VIVID_PUR);

  // Score bar
  int barWidth = (int)(sovMetrics.sovereigntyScore * 2.8);  // Max 280px = 100%
  uint16_t sovColor;
  if (sovMetrics.sovereigntyScore >= 80) sovColor = COLOR_VIVID_PUR;
  else if (sovMetrics.sovereigntyScore >= 60) sovColor = COLOR_CYBER_BLUE;
  else sovColor = COLOR_SUNRISE;

  tft.fillRect(12, y + 18, barWidth, 6, sovColor);

  // Score percentage
  tft.setTextDatum(TR_DATUM);
  tft.setTextColor(COLOR_WHITE);
  char sovStr[10];
  sprintf(sovStr, "%.0f%%", sovMetrics.sovereigntyScore);
  brFont.drawMonoText(sovStr, 300, y + 15, 1, COLOR_WHITE);
  ...
  char statusStr[60];
  int healthyCount = (navState.meshHealthy ? 1 : 0) + (navState.crmHealthy ? 1 : 0) + (navState.aiHealthy ? 1 : 0);
  sprintf(statusStr, "%d/3 live • Uptime: %s", healthyCount, getUptimeString().c_str());
  brFont.drawMonoText(statusStr, 160, y, 1, COLOR_WHITE);
...
void drawCurrentScreen() {
  switch (currentScreen) {
    ...
    case SCREEN_LINEAR:
      drawLinear();
      break;
    case SCREEN_SOVEREIGN_STACK:
      drawSovereignStack();
      break;
    case SCREEN_INFRASTRUCTURE:
      drawInfrastructureDashboard();
      break;
    case SCREEN_HOT_LEADS:
      drawHotLeads();
      break;
    case SCREEN_CEO_COMMAND:
      drawCEOCommandCenter();
      break;
    default:
      drawHomeScreen();
  }
...
void handleTouch() {
  ...
  switch (currentScreen) {
    ...
    case SCREEN_HOT_LEADS:
      // SYNC CRM button (bottom right: x=250-312, y=278-310)
      if (x >= 250 && x <= 312 && y >= 278 && y <= 310) {
        Serial.println("🔄 SYNC CRM button tapped - fetching live hot leads...");
        playBeep();

        // Show loading indicator
        tft.fillRoundRect(250, 278, 62, 32, 4, COLOR_SUNRISE);
        tft.setTextColor(COLOR_BLACK);
        tft.setTextDatum(TC_DATUM);
        brFont.drawMonoText("...", 281, 288, 1, COLOR_BLACK);

        // Fetch live CRM data
        bool success = fetchHotLeads();
        ...
        if (success) {
          ...
          // Orange flash for fallback to static data
          tft.fillRoundRect(250, 278, 62, 32, 4, COLOR_HOT_PINK);
          playBeep();
        }

        delay(200);

        // Redraw entire screen with updated data
        drawHotLeads();
        break;
      }
      break;
...
void connectWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("\n🔄 Initializing dynamic navigation...");
    initPerformanceMonitor();  // Initialize performance monitor
    initGenesisVerification();
    initSovereignStack();  // Initialize sovereign stack monitor
    initStaticMeshData();  // Start with static data
    initStaticCRMData();
    initStaticAIData();
    updateDynamicNavigation();  // Try to fetch live data
...
void handleSerialCommand() {
  if (Serial.available()) {
    ...
    if (cmd.startsWith("ALERT")) {
      ...
      // Quick heap stats
      Serial.println("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
      Serial.println("   HEAP MEMORY SNAPSHOT");
      Serial.println("━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
      Serial.printf("   Free:       %d bytes (%d KB)\n", perfMetrics.freeHeap, perfMetrics.freeHeap / 1024);
      Serial.printf("   Min Free:   %d bytes (%d KB)\n", perfMetrics.minFreeHeap, perfMetrics.minFreeHeap / 1024);
      Serial.printf("   Total:      %d bytes (%d KB)\n", perfMetrics.heapSize, perfMetrics.heapSize / 1024);
      Serial.printf("   Usage:      %d%%\n", getHeapUsagePercent());
      Serial.printf("   Fragment:   %d%%\n", perfMetrics.heapFragmentation);
      Serial.println("━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
    }
    else if (cmd == "WIFI") {
      // Quick WiFi stats
      ...
      if (WiFi.status() == WL_CONNECTED) {
        ...
        Serial.printf("   Quality:    %d%% (%s)\n", perfMetrics.wifiQuality, getWiFiQualityString().c_str());
      } else {
      ...
      Serial.println("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
      Serial.println("   BLACKROAD ESP32 SERIAL COMMANDS");
      Serial.println("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
      Serial.println("   ALERT <msg>  - Send emergency pager alert");
      Serial.println("   ACK          - Acknowledge current alert");
      Serial.println("   STANDBY      - Clear alerts and return to standby");
      Serial.println("   PING         - Check if device is responsive");
      Serial.println("   STATUS       - Show pager status");
      Serial.println("   PERF         - Full performance diagnostic report");
      Serial.println("   DIAG         - Alias for PERF");
      Serial.println("   HEAP         - Quick heap memory snapshot");
      Serial.println("   WIFI         - Quick WiFi status");
      Serial.println("   UPTIME       - Show uptime and boot reason");
...
void loop() {
  ...
  // Auto-refresh dynamic navigation every 5 minutes
  static unsigned long lastNavUpdate = 0;
  const unsigned long NAV_REFRESH_INTERVAL = 300000; // 5 minutes

  if (WiFi.status() == WL_CONNECTED && millis() - lastNavUpdate > NAV_REFRESH_INTERVAL) {
    updateDynamicNavigation();

    // Check alert conditions after navigation update
    #if ENABLE_ALERTS
      checkAllAlerts();
    #endif
